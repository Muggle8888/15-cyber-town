...
from scripts import f014_real_provider_uat as uat
...
def _patch_resources(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> tuple[Path, Path]:
    ...
    monkeypatch.setattr(uat, "LEDGER_DATABASE", ledger_root / "f-014.sqlite3")
...
def _ledger(ledger_root: Path) -> AcceptanceLedger:
    ledger = AcceptanceLedger(
        database_path=ledger_root / "f-014.sqlite3",
...
def _successful_replies() -> list[ProviderCompletion]:
    return [
        _completion("我会让暮光信号继续服务街区导览，在夜市灯光下标清路线。"),  # noqa: RUF001
        _completion("档案监听会保留这段信号和旧广播，成为持续可查的记录。"),  # noqa: RUF001
        _completion("雨夜信标能照亮街道，也能让投递路线更可靠。"),  # noqa: RUF001
        _completion("雨夜信标和街区导览很相配，也能让夜市灯光更有方向。"),  # noqa: RUF001
        _completion("我会把雨夜信标的选择写入信号档案，留下完整记录。"),  # noqa: RUF001
        _completion("雨夜信标会改善夜间路线，让投递与送达更加安全。"),  # noqa: RUF001
    ]


def test_f014_uat_fake_readiness_preserves_visible_text_scope_stage_and_budget(
...
def test_f014_uat_uses_one_bounded_semantic_retry_without_changing_visible_message(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    uat_root, ledger_root = _patch_resources(monkeypatch, tmp_path)
...
def test_f014_uat_stops_after_two_failed_semantic_retries(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    uat_root, ledger_root = _patch_resources(monkeypatch, tmp_path)
    ...
    with pytest.raises(uat.SemanticUatError, match="nia_briefing_context_not_understood"):
        asyncio.run(
            uat.run_uat(
                settings=_settings(),
                provider=provider,
...
def test_f014_offline_preflight_does_not_read_credentials_or_create_uat_resources(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    ...
    assert "F014_REAL_UAT_OFFLINE_PREFLIGHT=PASS" in output
    assert "max_retries=2" in output
    assert "cost_first_stop=true" in output


def test_f014_preflight_rejects_missing_authorization_without_creating_resources(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    uat_root, _ledger_root = _patch_resources(monkeypatch, tmp_path)
    monkeypatch.setattr(sys, "argv", ["f014_real_provider_uat.py", "--offline-preflight"])
