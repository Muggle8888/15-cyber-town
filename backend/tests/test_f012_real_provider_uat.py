...
from scripts import f012_real_provider_uat as uat
...
def _patch_resources(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> tuple[Path, Path]:
    ...
    monkeypatch.setattr(uat, "LEDGER_DATABASE", ledger_root / "f-012.sqlite3")
...
def _ledger(ledger_root: Path) -> AcceptanceLedger:
    ledger = AcceptanceLedger(
        database_path=ledger_root / "f-012.sqlite3",
...
def _successful_replies() -> list[ProviderCompletion]:
    return [
        _completion("夜市灯带会沿着主街延伸，我会从导览牌开始带你逛。"),  # noqa: RUF001
        _completion("我是 Nia，负责街区导览，也愿意帮你规划夜市路线。"),  # noqa: RUF001
        _completion("这段广播编号属于信号档案中的旧记录。"),
        _completion("我是 Ivo，负责整理信号档案和广播记录。"),  # noqa: RUF001
        _completion("这条雨夜路线仍用于投递，但标记已经褪色。"),  # noqa: RUF001
        _completion("我是 Rhea，负责雨夜投递，也会确保路线可靠。"),  # noqa: RUF001
    ]


def test_f012_uat_fake_readiness_preserves_visible_text_scope_stage_and_budget(
    ...
    assert [request.user_message for request in provider.requests] == [
        uat.CASES[0].context_message,
        uat._persona_message(uat.CASES[0]),
        uat.CASES[1].context_message,
        uat._persona_message(uat.CASES[1]),
        uat.CASES[2].context_message,
        uat._persona_message(uat.CASES[2]),
    ]
...
def test_f012_uat_uses_one_bounded_semantic_retry_without_changing_visible_message(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    uat_root, ledger_root = _patch_resources(monkeypatch, tmp_path)
    ...
    assert provider.requests[0].user_message == uat.CASES[0].context_message
    assert provider.requests[1].user_message == uat.CASES[0].context_message


def test_f012_uat_stops_after_two_failed_semantic_retries(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    uat_root, ledger_root = _patch_resources(monkeypatch, tmp_path)
    ...
    with pytest.raises(uat.SemanticUatError, match="nia_context_not_understood"):
        asyncio.run(
            uat.run_uat(
                settings=_settings(),
                provider=provider,
...
def test_f012_offline_preflight_does_not_read_credentials_or_create_uat_resources(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    ...
    assert "F012_REAL_UAT_OFFLINE_PREFLIGHT=PASS" in output
    assert "max_retries=2" in output
    assert "cost_first_stop=true" in output


def test_f012_preflight_rejects_missing_authorization_without_creating_resources(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    uat_root, _ledger_root = _patch_resources(monkeypatch, tmp_path)
    monkeypatch.setattr(sys, "argv", ["f012_real_provider_uat.py", "--offline-preflight"])
