...
## 2026-09-03：Step6独立QA发现P1并停止

状态：`step_6_blocked / awaiting_projection_integrity_fix_authorization`。42个不同用例41通过/1产品失败；另有2次本轮新QA构造失败已纠正并保留证据。S6-QA-001复现ledger15→16、4行projection计数被调低后admission未拒绝、active permit1。缺陷库保留，不recovery或顺带修复。

新QA两文件ruff/format/mypy通过；ignore/sensitive、59untracked whitespace、130新SQLite integrity/FK通过；63trace/882stage、非14stage组0、open trace0、非零cost0；91metadata surfaces sentinel0。原94文件SHA256仍0094cdb520fe8406f764124db4a35e2282e810def7c734d3b52be6bb90b8dc27，13migration不变。

正式main五文档/0untracked/0staged；功能37tracked/59untracked/0staged；HEAD/origin/main不变。新根138文件/52后代目录/32,140,348bytes保留；V30—V33不存在、tooling及其余旧根保留；正式三库和sidecar不存在、8000空闲。完整quality/evaluator/三进程digest/性能未运行，不用历史结果代替。下一动作仅单独修复授权，不能申请Step7；详见[evidence](evidence.md)。
...
- 正式main=5tracked/0untracked/0staged；功能指定分支=37tracked/57actual untracked/0staged。两处HEAD/本地origin/main均为1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3。
- 当前94文件只读快照SHA256=`0094cdb520fe8406f764124db4a35e2282e810def7c734d3b52be6bb90b8dc27`；写入前五文档缺少28个当前hash。V9旧83表=63相同/20不同/11新增，不能当成当前基线。
- 13migration匹配、tracked/untracked whitespace通过；正式三库/sidecar不存在，8000空闲。
- S6-PRE-001（P1验收追溯）与S6-PRE-002（P2 runner新根边界）开放，未确认产品缺陷。动态测试=0，quality/evaluator/三进程digest/性能全部未运行；无新skip、模型调用、费用或数据库连接。
- 仅更新正式五文档；功能94文件未改、QA根未创建；V30—V33不存在，保留其缺失状态，其余旧根/worktree/分支/缓存保留。准确快照、资源和恢复Prompt见[evidence](evidence.md)；不能申请Step7。

## 2026-09-02：F-009 Step 5全部完成

- V33 failure-first：projection候选、steady-profile污染和v6断言共3 failed；最小修复后2 passed/33 expected skipped。
- 历史V14—V19实验仅在显式专用批次下运行，普通quality不连接或复用旧证据；缺失配置使用固定脱敏skip原因。
- steady-profile在预置`_INDEPENDENT_CLIENT=true`污染下仍通过，并在结束后恢复原值；migration测试验证当前注册末版本v7。
- 完整`scripts/quality.py`全绿：ruff、129文件mypy、schema、Godot import/unit、connectivity 9/9、Dialogue 10基础+Multi-NPC、pytest `1997 passed / 133 skipped`、ignore/sensitive final全部通过。
- 全仓format为129 files already formatted；tracked diff-check、57个untracked no-index whitespace、11份migration登记哈希、正式数据库与端口门禁通过。
- 状态：`step_5_complete / awaiting_step_6_authorization`。Step6、Git交付、部署和资源删除均未执行。

## 2026-09-02：F-009 Step 5 Dialogue runner修复通过，完整pytest仍有22项测试基础设施失败

- failure-first：旧`invalid_recovery`单场按预期失败，provider调用仅1次；最小test-only修复后新`provider_invalid_response`单场通过且不可Retry。
- Dialogue loopback：10个基础场景和Multi-NPC全部通过；unavailable/timeout恢复、malformed-response及多轮记忆场景保持既有契约。
- quality已通过ignore/sensitive preflight、lock、ruff、129文件mypy、schema、Godot import/unit、connectivity 9/9和Dialogue loopback。
- 全量pytest：`2008 passed / 100 skipped / 22 failed`。失败分类为历史projection候选20、steady-profile runner接口1、storage migration v6断言1；均不在本轮授权修改范围，尚无新产品缺陷证据。
- full format：允许文件已格式化；另有4个未授权文件仍失败。tracked diff check与57个untracked no-index whitespace检查通过。
- 状态：`step_5_comprehensive_acceptance_blocked / awaiting_remaining_test_harness_authorization`。Step5/R-10未完成，Step6未进入。

## 2026-09-02：F-009 Step 5 harness主体已修复，Dialogue test runner待授权

- failure-first：4 failed；最小修复后4 passed。v7断言更新，V7/V8/V9/V10历史runner无专用全新根时安全skip，环境值未再出现在异常。
- 综合磁盘/故障回归：412 passed / 100 skipped；evaluator 19 passed，固定25/25 case及三进程digest保持通过。
- 最新quality已通过ignore/sensitive、lock、ruff、129文件mypy、schema、Godot import/unit和connectivity 9/9；此前暴露的均为测试/运行器基础设施问题，已在测试范围内最小收口。
- 剩余Dialogue loopback发现冻结契约冲突：`provider_invalid_response`为`retryable=false`，旧Godot integration runner却要求所有失败均显示Retry。Python-only候选失败后已回退，未绕过场景。
- 状态：`step_5_comprehensive_acceptance_blocked / awaiting_test_only_godot_runner_authorization`；Step5、R-10未完成，Step6未进入。

## 2026-09-02：F-009 Step 5 综合验收首次执行被 harness 阻塞

- evaluator 专项 `19 passed`；固定25/25 case、11维度、三进程输出与digest完全一致，未授权dispatch、scope泄漏、禁止原文、重复计费/写入、限流绕过、retry amplification和非零成本均为0。
- 综合磁盘/故障回归 `440 passed / 48 failed / 60 deselected`。47项为V7/V8/V9历史诊断测试的固定根/专用环境依赖，1项为control migration v6过期断言；当前没有证据确认新产品缺陷。
- 按硬门禁停止，未运行完整质量入口或Step6。失败输出曾由测试框架展开进程环境映射，任何值均未写入summary、文档或Git；建议轮换可能暴露的凭据。
- 新根终盘为 `211 files / 193 descendant dirs / 43,295,234 bytes`，含205 SQLite和6个metadata-only JSON，reparse/`.env*`/nested Git均为0；保留等待修复重验及Step5收口，Codex不删除。
- 状态：`step_5_comprehensive_acceptance_blocked / awaiting_test_harness_fix_authorization`。

## 2026-09-02：V29 门禁修订及唯一核心矩阵通过

- 失败优先：旧实现下 V29 专项 `6 failed / 1 passed`；实施后契约 `39 passed`。两次中间 green 仅暴露测试样本的 nearest-rank 构造问题，未扩大产品范围。
- 必要回归：主批 `182 passed / 2 environment failures / 53 deselected`；两项专项在分离 pytest basetemp 与预建数据根后 `2 passed`，确认不是产品失败。静态/格式/类型/whitespace/migration门禁通过。
- 唯一矩阵：in-memory p95/p99=`1.7970/2.3656ms`、paired delta=`-0.1350ms`、吞吐比=`111.69%`；HTTP on p95/p99=`184.5241/209.3907ms`、相对增量=`116.1614ms`；SQLite observability与reject均通过。`failure_codes=[]`、`warning_codes=[]`。
- 数据审计：66 SQLite integrity failures=0、FK violations=0；1,200 trace/16,800 stage，14-stage缺失=0；成本、token、未释放permit与未settled状态均为0。正式数据库不存在，8000端口空闲。
- 状态：`step_5_core_revalidation_complete / awaiting_step_5_completion_authorization`。未运行 Step 5 综合验收、Step 6 或 Git 交付。

## 2026-09-02：V29 in-memory 门禁修订已授权并完成资源预登记

- 正式目录为 main 且仅五份项目文档修改；功能目录为 `feat/f-009-safety-cost-performance`，`33 tracked modified / 57 actual untracked / 0 staged`；两处 HEAD/local origin/main=`1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3`。
- V28 权威摘要 SHA256=`B0A0D80C87FF9A0194633E386EAEDE33A862E2B4041E7279FDEE5475E3B3500D`；13 份 migration 指纹保持登记值。
- V29 契约已获准：recorded p95 `>3.0ms` warning、`>3.5ms` fail；paired p95 delta `>0.5ms` warning、`>0.75ms` fail；p99 `5ms` 与 throughput `80%` 不变。
- 新根及完整 finite surface 已在创建前展开、打印和登记。当前状态：`step_5_v29_gate_revision_authorized / failure_first_pending`。

## 2026-09-02：V29 Step 0 in-memory 门禁只读评审草案

- 未修改产品、契约、测试、migration或阈值，未运行新矩阵/服务、未创建资源。
- V28 recorded p95五轮median/MAD/3MAD上界/最大=`2.844435/0.089335/3.112440/3.204800ms`；paired delta median/MAD/3MAD上界=`0.210025/0.100125/0.510400ms`。
- 维持2ms需要无收益证据的大型control状态机重构；已排除savepoint、GC和scope-tag三个小候选。推荐在相同profile下采用recorded p95 `3.0 warning/3.5 fail`及paired p95 delta `0.5 warning/0.75 fail`，保留p99 5ms、吞吐80%及其他全部门禁。
- 推荐值尚未授权或写入代码；状态继续为`step_5_v28_core_gate_failed / awaiting_in_memory_gate_review_authorization`。

## 2026-09-02：V28 当前产品路径复验与根因边界完成

- 发现 V27 in-memory benchmark 仍调用 V24 前的分解式 control API；新增失败优先契约后改为当前产品三-fence API。red=`1 failed`；runner 路由中间暴露 synthetic nested transaction 缺陷，修正后 `5 passed`；完整性能契约 `32 passed`，ruff/format/mypy通过。
- 唯一一次统一 1 warm-up + 5 measured 核心矩阵完成：no-recorder control p95/p99=`2.7237/3.1882ms`，in-memory recorder=`2.8436/3.6704ms`，吞吐比=`104.33%`；唯一失败码=`in_memory_p95_exceeded`。
- 正式默认 HTTP off/on p95=`169.9669/234.0655ms`，on p99=`250.1393ms`、吞吐=`18.029168/s`，增量=`64.0986ms`；control growth=`262144`。问题二、SQLite recorder、reject、dispatch/成本/完整性门禁保持通过。
- 五轮 p95 波动范围 baseline=`2.1462..3.0447ms`、recorded=`2.2561..3.2048ms`；两者共同漂移，recorder 不是阻塞来源。
- 每 execution 成功路径为78条SQLite statement，含7组savepoint。仅诊断的无savepoint成功路径五轮p95仍=`2.3793ms`，且会破坏异常隔离，不能落地；没有证据支持继续无边界产品微优化。
- non-gating排除项：GC enabled/disabled p95=`2.5262/2.4591ms`；scope tag current/reuse p95=`2.6358/2.5680ms`，reuse吞吐反而从`730.147/s`变为`719.948/s`。两项收益约0.067ms且落入波动，均未改产品。SQL阶段计数为ingress/admission/finalization=`6/43/29`。
- `2ms` 来自 Step 0 fake-only设计预算、不是生产SLA，并早于当前durable三-fence路径。未擅自改阈值；状态=`step_5_v28_core_gate_failed / awaiting_in_memory_gate_review_authorization`，等待用户单独批准门禁适用性评审。
- V28根为211 files/83 dirs/46,364,362 bytes，reparse/env-like/nested Git=0；正式三数据库不存在，8000未监听。资源保留，Codex不删除。

## 2026-09-02：V27 Step 5 核心性能复验失败并停止

- 预检通过：正式 main 仅五份项目文档；功能分支为指定 F-009 分支，`33 tracked modified / 57 actual untracked / 0 staged`；两处 HEAD/local origin/main=`1a4fc2c`。
- 0001—0007 control、0001—0004 observability 和业务 0001/0002 migration 指纹与登记一致；正式三数据库不存在，8000 未监听。
- 失败优先 runner 根契约 red=`1 failed`、green=`1 passed`；最终核心契约 `8 passed`，两文件 ruff/format/mypy 通过。
- 唯一核心失败码=`in_memory_p95_exceeded`：recorded p95/p99=`2.3465/3.2453ms`，no-recorder control p95=`2.4102ms`，配对吞吐比=`102.03%`。失败是共同控制链 p95，不是 recorder 独有成本。
- HTTP control-on=`217.294/241.6763ms`、`19.124091/s`，相对增量=`69.2078ms`；control growth=`262144`。SQLite observability、三类 reject 与所有其他阻塞门禁通过；three-SQLite 仅保留两个非阻塞延迟诊断码。
- 66 个新 SQLite integrity=`ok`、FK violations=0；1,200 trace / 16,800 stages，summary sensitive sentinel 命中0。正式数据库不存在，8000 未监听。
- 状态：`step_5_core_revalidation_failed / awaiting_in_memory_gate_decision`；性能 P1、Step 5、R-10 继续开放，未进入综合验收或 Step 6。

## 2026-08-31：V26 产品 Gate 2 空间门禁通过

- memory runner 红测 `1 failed`；最小 connection routing 修复后 `3 passed`，格式化后复验 `3 passed`。两次 green 均未创建磁盘 SQLite。
- Gate 2 定向回归：`118 passed / 53 deselected`；53 项为明确排除的 V10 独立候选专项。
- 五对空间：每对 `266240 -> 258048 bytes`，saving=`8192`，candidate index pages=`3 -> 0`，逻辑 digest 相等。
- 产品 schema：baseline v6、product v7；WAL/FULL、integrity=`ok`、FK=0，open/unsettled/nonterminal/nonzero-cost 均为0。
- summary SHA256=`F3DCD439A11F9A3F0E5264C1DDFB026C9B121F367F052A2A312543BF320B0539`，status=`product_space_gate_passed`，failures=`[]`。
- 当前授权范围 ruff/format/mypy、两目录 diff check、0001—0007 migration hash、正式数据库和8000端口检查通过。
- 状态：`step_5_problem_2_space_gate_resolved / awaiting_step_5_core_revalidation_authorization`。尚未运行完整性能矩阵、Step 5 综合验收或 Step 6。

## 2026-08-31：V26 产品 Gate 2 memory runner 隔离复核失败

- 测试断言：仅把 migration/user_version 的 v6 期望更新为 v7，未修改预算业务契约。
- 隔离批次：全新 `pytest-followup-01` 运行两项 memory contract，结果 `2 failed`，稳定报 `control_execution_intents` 不存在。
- 根因：`_MemoryControl.initialize()` 迁移 `:memory:` connection；execution-intent recovery 的 inherited composed transaction 却通过 `_writer` 打开磁盘占位库。两个连接 schema 不一致。
- 判定：synthetic benchmark runner 缺陷，非产品 0007/migration 缺陷。按硬停止条件未运行其他回归、五对空间门禁或静态门禁。
- 状态：`step_5_problem_2_v26_product_gate_2_memory_runner_blocked / awaiting_benchmark_runner_fix_authorization`。8KiB 空间问题仍未形成产品 Gate 2 最终结论。

## 2026-08-31：V26 产品 Gate 2 定向回归触发硬停止

- 失败优先：产品实现前 8 项专项为 `8 failed`；注册 migration 7、增加精确 0007 及授权测试/runner 后为 `8 passed / 35 deselected`。
- 产品边界：0007 仅删除 `idx_budget_owners_npc` 并设置 `user_version=7`；repository 逻辑未改变，既有 migration 未改写。
- 扩大回归：`113 passed / 57 failed`。分类为旧 v6 断言 2 项、缺少 V10 专用环境的历史候选专项 53 项、待隔离确认的 memory contract 2 项。
- 按硬停止条件未运行五对 fresh 空间批次；`space-gate-01`、`space-pair-01..05` 和 `summary.json` 均不存在。
- 状态：`step_5_problem_2_v26_product_gate_2_regression_blocked / awaiting_test_scope_authorization`。产品候选保持未提交；Step 5、R-10 和 8KiB 空间阻塞继续开放。

## 2026-08-31：V26 query-plan Gate 1 修复通过

- 红测：新 query-plan fix API 不存在，1 collection error；实现后纯契约 `11 passed`。
- 唯一 query-plan-only 批次：baseline/candidate 使用独立数据库；DROP 已 commit 并关闭写连接，candidate 由全新只读连接检查。
- 结果：baseline schema/index-list/index pages=`true/true/1`；candidate=`false/false/0`。baseline active-NPC 使用候选索引，candidate 改用 quota-time + owner PK；三个正常预算查询计划逐项不变。summary status=`query_plan_gate_passed`、failures=`[]`。
- 工程门禁：11 passed；两文件 ruff/format/mypy、两目录 diff check 与 untracked no-index whitespace 通过；0001—0006 哈希不变；新库 integrity=`ok`；正式数据库不存在，8000 空闲。
- 新根终盘：`7 files / 6 descendant dirs / 362790 bytes`，含 2 SQLite、2 WAL(0 bytes)、2 SHM(32768 bytes) 与 1 metadata-only summary；reparse/`.env*`/nested Git/禁止值命中均为0，保留至 Step 5 收口由用户手动删除。
- 没有运行 history、空间五对、完整性能矩阵、Step 5 综合验收或 Step 6；产品 0007 不存在。
- 状态：`step_5_problem_2_v26_query_plan_gate_1_complete / awaiting_revised_product_gate_2_authorization`。

## 2026-08-31：V26 synthetic Gate 1 失败并停止

- 红测：诊断模块缺失导致 1 collection error；green-01 因 Windows raw-string 语法错误停止；修正后 green-02/03/04 均为 5 passed，最终 ruff/format/mypy/diff 通过。
- query plan：三个正常预算查询均不使用候选索引；baseline NPC 异常检查使用该索引，但同连接 DROP 后 EXPLAIN 仍报告该索引，失败码=`candidate_query_plan_retained_dropped_index`。
- history：100/1,000/10,000 baseline/candidate 语义、fail-closed、recovery、integrity/FK 均通过；candidate missing lookup p95=`0.23/1.09/27.79ms`，均低于 50ms。
- 空间：五对均为 `266240 -> 258048 bytes`，每对 occupied saving=`8192`；candidate candidate-index pages=0，逻辑 digest 与归属一致。
- 产品 0007 不存在；状态=`step_5_problem_2_v26_synthetic_preflight_failed / awaiting_query_plan_probe_fix_authorization`。不进入 Gate 2、完整矩阵或 Step 6。

## 2026-08-31：V26 独立 8KiB control 空间修复启动

- 预检通过：正式 main 仅五文档；功能 `33 tracked modified + 54 actual untracked + 0 staged`；两处 HEAD/local origin/main=`1a4fc2c`。
- 125/140ms control overhead 契约保持；control 0001—0006 SHA-256 与既有 evidence 一致；产品 0007 和 V26 根均不存在。
- 当前只进入 synthetic Gate 1；候选仅 `idx_budget_owners_npc`，不得扩大到第二个索引或产品 schema。
- 状态：`step_5_problem_2_v26_space_preflight_authorized / failure_first_pending`。

## 2026-08-31：Step 0 control-on 门禁修订实施完成

- 功能契约现为 `>125ms` warning、`>140ms` fail；warning 不阻断退出码，绝对 HTTP 与其他冻结门禁不变。
- 红测：缺常量/evaluator 导致 collection error；最终边界与相关契约 `5 passed / 23 deselected`。
- ruff、format、mypy、tracked diff check 通过；未运行新矩阵、服务或 SQLite。
- 既有 V24 增量 `117.1586ms` 在新契约下无 warning/failure；仅为旧证据重解释，不是新测量。
- 状态：`step_0_control_overhead_gate_revision_complete / awaiting_8kib_space_fix_authorization`。问题二仍被 8KiB control 空间超限阻塞，Step 5/R-10 未完成。

## 2026-08-31：Step 0 control-on 相对 p95 门禁重审完成（历史决策记录）

- 只读复核 V13 E/C、V21、V22、V24、V25；未运行新矩阵或修改产品。
- 原 30ms 为 Step 0 设计预算，无项目实测基线支撑；五组真实五轮汇总增量均失败，范围 `86.3302—125.2705ms`。
- 统计结果：median=`98.1198ms`、MAD=`11.7896ms`、median+3MAD=`133.4886ms`；当前 V24 产品基线=`117.1586ms`。
- 建议候选：相同 profile 下 `>125ms` warning、`>140ms` fail；绝对 `250/400ms`、吞吐、空间和全部语义门禁不变。
- 该轮状态为 `step_0_control_overhead_gate_review_complete / awaiting_threshold_revision_authorization`；随后用户已批准并完成 125/140ms 契约实施。8KiB 空间阻塞、Step 5 和 R-10 继续开放。

## 2026-08-31：V25 三-fence 有界归因停止并收口

- V25 三次诊断批均在证据形成前 fail-closed：缺 `dbstat`、并发序号错误关联、worker `ContextVar` 无法回传请求 task。没有第四批，没有 `summary.json`，没有产品改动。
- V24 可信聚合证据：control-on 相比 off 每 100 请求增加 304 次 writer/path lease 和 301 次 write commit；新增连接数为 0，writer wait p95 近零。三次 WAL/FULL fence 与路径检查是结构性热点，但 30ms 不可达尚未取得逐请求严格证明。
- 空间已完成页级归属：最差 occupied growth 66 页，门禁 64 页；54 页表、6 页必要约束索引、6 页非唯一 owner scope 索引。超限精确为 2 页/8192 bytes，无未知泄漏。
- 状态：`step_5_problem_2_v25_attribution_blocked / awaiting_architecture_or_step_0_decision`。停止微优化；延迟在架构调整与 Step 0 门禁重审间选择，空间候选另行预验证。问题二、性能 P1、Step 5、R-10 均未完成。

## 2026-08-31：V24 execution-intent 产品实施启动

- Gate 1 已通过：append-only `0006`、repository/application 三-fence 状态转换、取消/restart/replay/waiter/迟到、真实三库归属及 v5→v6 migration 回归全绿。
- 红测 `2 failed`；产品回归批次为 `2 + 3 + 7 + 158 + 20 passed`（53 deselected 仅旧 V10 候选专项）；ruff、format、mypy、diff、migration、integrity/FK、隐私和正式数据库门禁通过。
- Gate 2 最终失败：control-on p95=`189.184ms`、control-off p95=`72.0254ms`，增量=`117.1586ms > 30ms`；control growth=`270336 > 262144 bytes`。绝对 p95/p99、吞吐、dispatch、integrity/FK 和隐私门禁通过。
- 状态：`step_5_problem_2_v24_performance_gate_failed / awaiting_next_authorization`。产品三-fence语义保留；问题二、性能 P1、Step 5 与 R-10 继续开放，未进入综合验收或 Step 6。
- Gate 2 首次启动因独立客户端模块搜索路径缺失在首个预热请求前终止；无性能样本。失败现场保留，改用全新 `tcp-1plus5-retry-01`，不得复用首次数据库。

## 2026-08-31：V23 execution-intent synthetic 预验证通过

- 红测：缺 test-only store 时 V23 `6 failed`。
- 最终测试：完整 architecture probe `16 passed`；ruff、format、mypy 通过。
- semantic：`synthetic_candidate_passed`，summary SHA256=`BC7F0C81D8C3BE66A53F2126F5F8C00930562EF02768EBD5C9738747599462AA`。
- 不变量：三 fence；6 execution/6 quota/6 intent/5 settled/1 released；2 conservative；重复 dispatch/settlement/business claim=0；integrity/FK/privacy 全绿。
- 边界：产品 `0006` 不存在；V21 产品文件与既有 migration 未变；未运行性能矩阵。
- 状态：`step_5_problem_2_v23_synthetic_preflight_passed / awaiting_product_architecture_authorization`。问题二、性能 P1、Step 5、R-10 继续开放。

## 2026-08-31：V23 execution-intent synthetic 预验证启动

- V22 已回退且 V21 语义/静态门禁保持通过。
- 用户仅为 synthetic 候选接受 execution quota 时点后移和 intent 后崩溃保守结算；产品语义尚未变更。
- V23 根 canonical path、父目录及 `E:\Agent` 父链已只读验证：目标不存在，父链 reparse=0。
- 当前状态：`step_5_problem_2_v23_synthetic_preflight_authorized / failure_first_pending`；问题二、性能 P1、Step 5、R-10 继续开放。

## 2026-08-31：V22 精确回退完成，转入单请求 durability 架构决策

- 仅撤销六个授权文件中的 V22 group-commit 增量；未 reset/stash/checkout，未影响问题一 `0005`、V21 或其他 F-009 实现。
- V22 symbol 命中 0；回退后 SHA256 已登记到 evidence。
- 回退验证：`153 passed / 55 deselected`；ruff、format、mypy、diff check 通过。
- Git：功能分支 `feat/f-009-safety-cost-performance`，HEAD/local origin/main=`1a4fc2c`，`33 tracked modified + 53 actual untracked + 0 staged`；正式 main 仅五份项目文档修改。
- 架构结论：在 WAL/FULL、provider 前后精确状态、取消/restart 语义及无 provider receipt 的条件下，五个 fence 不能等价压缩。推荐候选 A 可把正常路径降为三个 fence，但需要单独批准 admission 时点和 crash-window 保守结算语义；否则应重审 30ms 门禁。
- 状态：`step_5_problem_2_v22_rolled_back / awaiting_single_request_durability_architecture_decision`；性能 P1、Step 5、R-10 继续开放。

## F-009 Step 5 问题二 V22 group-commit门禁失败（2026-08-31，当前权威状态）

状态：`step_5_problem_2_v22_core_gate_failed / awaiting_rollback_or_architecture_decision`。

- 红测确认两个并发请求原为`10 commit / 10 path-check`；V22后同fence共享物理事务专项及item/physical故障分流为`5 passed`。
- 首轮综合语义发现breaker item失败误回滚budget settlement；最小修复后完整集合`158 passed / 55 deselected`，取消、幂等、预算、breaker、重启和异步持久化语义通过。
- 固定真实TCP 1+5：off p95/p99=`104.4321/170.3675ms`，on=`229.7026/298.6951ms`，吞吐=`26.842997/16.201588 per second`；绝对门禁通过。
- 相对增量=`125.2705ms`，允许=`30ms`，唯一失败码`control_on_relative_p95_regression`。measured on每100请求仍有`590—598`次write commit、`492—500`次path check，证明逐请求工作量不能靠跨request group-commit消除。
- V22候选保持未提交，未自动回退；问题二、性能P1、Step5与R-10继续开放，未进入综合验收或Step6。

## F-009 Step 5 问题一预算历史增长修复完成（2026-08-31，当前权威状态）

状态：`step_5_problem_1_budget_history_growth_resolved / problem_2_and_step_5_still_open`。

- 根因：旧预算 admission 每次聚合最近24小时 ledger；1000行时约52,107 VM steps，属于明确 O(n) 历史扫描。
- 产品修复：append-only control `0005` + repository rolling projection；ledger继续是唯一权威数据，projection可重建，外部协议不变。
- 有界证据：100/1000行 lookup=`188/178` VM steps；历史扩大10倍没有增加查询工作量。
- 语义/回归：red 2项按预期失败；green 2项通过；生命周期3项、预算相关69项、控制/migration相关109项通过。重复计费/重复写入/scope泄漏为0。
- 工程门禁：ruff、format、mypy、diff check、旧migration hash、integrity/FK、正式三库不存在通过；8000由既有外部Python进程占用，本轮未启动或处置服务。
- 问题二与整链 p95 仍开放；本结论不关闭性能P1、Step5或R-10。

## F-009 Step 5 V20 固定成本归因完成（2026-08-31，当前权威状态）

状态：`step_5_v20_fixed_cost_attribution_complete / awaiting_control_state_boundary_design_authorization`。

- 失败优先：V20 缺少契约时 `3 failed`；实现后 `3 passed`。首次正式批次完成12000 execution 后在五轮聚合器触发未定义变量，空 `attribution-01` 保留；聚合回归补齐后 V9/V10/V20 工具集 `34 passed`，全新 `attribution-02` 完成同协议批次。
- baseline 五热点合计均时 `1.056843ms`：transaction `0.031866`、SQL `0.309551`、HMAC/DTO `0.138308`、Python orchestration `0.573922ms`；recorded 合计 `1.114199ms`，同类为 `0.032906/0.321674/0.174904/0.594607ms`。
- baseline 五轮 p95=`2.4310/2.4587/2.4424/2.3405/2.0954ms`；recorded=`2.9592/2.8821/2.8030/2.1658/2.2907ms`。这是带探针 non-gating 数据，不替代 V19 plain p95。
- 结论：scope tag 复用只覆盖约13%均时，简单事务框架只约3%；主成本为 SQL约29%与 Python状态校验/编排约54%。下一步进入架构设计而非继续 SQL 微调或恢复 V12 批事务。
- 工程门禁：定向 `34 passed`；ruff、format、mypy、tracked/untracked whitespace通过；隐私0、正式数据库不存在。端口8000存在一个2026-08-30启动、命令不含cyber-town/F009/uvicorn的外部Python监听，V20未启动或处置该进程。

## F-009 Step 5 V19 single-write 门禁失败（2026-08-31，当前权威状态）

状态：`step_5_v19_single_write_performance_gate_failed / awaiting_next_performance_decision`。

- V18 失败归因保持：每 1000 execution 的 expiration 为 1998 logical batch/3996 statements，lifecycle delta 为 2000 batch/4000 statements。
- V19 只改 synthetic 候选 SQL 形状：无过期行不写 totals；reserve/settlement/release 每个 logical delta 只执行一条写语句。
- 新根与所有实际子路径已先登记；产品 repository、SQL migration、Dialogue v1、Godot 和阈值不变。
- validation-01 在时钟回退重建前因 runner `AttributeError` 停止；未生成摘要或进入空间/性能。保留不复用，下一批次固定为全新 validation-02。
- validation-02：16 reserve/settlement、hour/day/rollback、integrity/FK 全绿；因未安排 no-expiration 场景而 skip counter=0，被 harness 误判并在 space 前停止。validation-03 将显式覆盖该门禁。
- validation-03：语义四 checkpoint、空间 `196608<=262144`、稳定性、吞吐比 `0.922352`、recorded p99=`4.2001<=5ms`、隐私0均通过；SQL 数为 delta 2000、expiration probe/update/skip=`999/940/59`。
- 失败：baseline p95=`2.6911ms`、recorded p95=`3.0901ms`；reservation=`1.0247ms`（相对 V17 -5.14057%），settlement=`0.4372ms`（改善4.248795%）。产品0005=false，性能P1/Step5/R-10继续开放。
- 工程门禁：最终 `33 passed`；ruff、format、mypy、tracked/52 untracked whitespace、10 migration、正式数据库与端口边界检查通过。

## F-009 Step 5 V18 revised performance 门禁失败（2026-08-31，当前权威状态）

状态：`step_5_v18_revised_performance_gate_failed / awaiting_revised_projection_performance_decision`。

- 失败优先：red=1 collection error；green 首轮仅 historical validator 预期文本不符，修正后 `28 passed`；ruff/format/mypy/diff check 通过。
- 空间：before/after=147456/344064 bytes，增长 196608<=262144，integrity/query plan 正常。
- plain 五轮聚合：baseline p95/p99=2.9093/3.9134ms；recorded=3.1542/3.7507ms；吞吐比 0.944995，稳定性通过。recorded p95 失败。
- stage：reservation=1.0807ms，相对 V17 -10.8865%；settlement=0.4132ms，仅改善 9.5050%。两项降幅门禁失败。隐私0，产品0005=false。

## F-009 Step 5 V18 revised performance 启动（2026-08-31，当前权威状态）

状态：`step_5_v18_revised_performance_authorized / prevalidation_pending`。预检通过：semantic summary status=passed、SHA256=`65BBB6EA...A0B4`；正式五文档；功能 33 tracked + 52 actual untracked、staged 0；10 migration，产品 0005 不存在；新根不存在、父链非 reparse。

## F-009 Step 5 V18 revised semantic 通过（2026-08-31，当前权威状态）

状态：`step_5_v18_revised_semantic_passed / awaiting_set_based_performance_prevalidation_authorization`。

- red：缺 revised symbols 的 1 个 collection error；green：`27 passed`。ruff、format、mypy、两目录 diff check 通过。
- 磁盘 semantic：16 reservations、16 settlements；hour/day exact cutoff 与 clock rollback rebuild 三 checkpoint 均 projection match、integrity ok、FK 0；禁止原文命中 0。
- 性能执行为 false，产品 0005 为 false。正式数据库不存在；Git 仍为正式五文档、功能 33 tracked + 52 actual untracked、staged 0。

## F-009 Step 5 V18 revised semantic 启动（2026-08-31，当前权威状态）

状态：`step_5_v18_revised_semantic_authorized / zero_seed_update_pending`。预检通过：正式五文档；功能 33 tracked + 52 actual untracked、staged 0；两处 HEAD/origin 一致；10 migration 未增，产品 0005 不存在。旧 V18 数据库 SHA256 仍为登记值，新根不存在、父链非 reparse。

## F-009 Step 5 V18 synthetic 语义门禁失败（2026-08-31，当前权威状态）

状态：`step_5_v18_synthetic_semantic_gate_failed / awaiting_revised_set_based_candidate_authorization`。

- red：缺少 V18 symbols，1 个 collection error；实现后最终定向 `25 passed`。两文件 ruff、format、mypy 通过。
- preflight：semantic 前置完成；space workload 在窗口过期负 delta 的 CHECK 前置校验处安全失败并回滚。未生成 summary；plain 1+5、stage companion 执行数均为 0。
- 产品 0005 不存在；正式数据库不存在；Git 仍为正式五文档、功能 33 tracked + 52 actual untracked、staged 0。性能 P1、Step 5、R-10 未完成。

## F-009 Step 5 V18 set-based 候选启动（2026-08-31，当前权威状态）

状态：`step_5_v18_set_based_projection_authorized / synthetic_preflight_pending`。预检通过：两目录 HEAD/local origin/main 均为 `1a4fc2c`；正式仅五份文档；功能 33 tracked modified、52 actual untracked、staged 0；10 migration、V16/V17 摘要哈希一致；产品 0005 与 V18 根不存在。已登记 V18 根和 surface，尚未创建资源或执行红测。

## F-009 Step 5 V15 测量协议与归因完成（2026-08-30，当前权威状态）

状态：`step_5_v15_protocol_attribution_complete / awaiting_next_performance_authorization`。

- red=`3 failed / 38 passed`；实现后两次定向均`41 passed`。四文件ruff、format、mypy、untracked whitespace通过。
- synthetic/preflight与正式核心矩阵共同引用同一协议配置；旧AB/BA无warm-up判定不再作为后续产品门禁依据。
- plain逐轮metadata和non-gating stage companion已接入核心矩阵输出；有界验证24,000 fake-only execution完成，summary SHA256=`EB98CF406D251C366802E3C6A1871BAC045ACD1AFAB3674FE8C103A8920583D7`。
- 当前v4基线中budget reservation仍是最大stage热点；结果只验证测量/归因，不代表rolling projection或产品0005通过。
- 功能目录仍33 tracked modified/52 actual untracked/staged0，正式main仍仅五文档，旧migration diff=0，正式三数据库不存在。性能P1、Step5、R-10继续开放。

## F-009 Step 5 V14 产品 0005 核心门禁失败（2026-08-30，当前权威状态）

状态：`step_5_v14_product_core_gate_failed / awaiting_next_performance_authorization`。

- 产品红测按预期因缺少 0005/投影契约失败；实现后产品专项 `18 passed`，相关预算、迁移、retry/breaker、preflight 与性能契约兼容回归 `168 passed`。
- 唯一完整矩阵未通过：in-memory p95=`2.2077ms`；默认 HTTP control-on p99=`412.2446ms`；control-on/off p95 增量=`102.9848ms >30ms`。SQLite observability 与拒绝路径门禁通过。
- 按任务卡停止条件没有执行 Step 5 综合验收；已精确撤销本轮产品 0005、repository、产品测试和 benchmark 改动。产品 0005 不存在，旧 migration diff=0，回退后定向 `149 passed`、静态门禁通过。
- 功能目录恢复为 33 tracked modified / 52 actual untracked / staged 0；正式目录仍仅五份项目文档修改。正式三数据库不存在；本轮随机本地端口已释放，8000 仍由同级 travel-assistant 的既有进程占用，非本轮创建。
- 性能 P1、Step 5、R-10 均未完成；下一步等待新的有界性能方案授权，不进入 Step 6。

## F-009 Step 5 V14 corrected projection p95 有界补证（2026-08-30，当前权威状态）

状态：`step_5_v14_p95_attribution_passed / awaiting_product_0005_implementation_authorization`。

- 预检通过：正式main仅五文档；功能33 tracked/52 untracked/staged0；两处HEAD/origin/main=`1a4fc2c`；V14-v2摘要、10migration哈希不变，产品0005不存在。
- 红绿：新增AB/BA顺序和4/5共同阻塞判定后collection red；实现后`16 passed`，recorder factory类型修正后独立复核仍`16 passed`。ruff、format、mypy全绿。
- 唯一补证：AB/BA/AB/BA/AB，A/B各5轮×1000 execution。recorded聚合p95/p99=`1.9879/2.62ms`、吞吐`945.477079/s`；baseline吞吐`910.827609/s`，ratio=`1.038042`。recorded/baseline逐轮p95超限分别2/5和5/5，共同超限2/5，未形成稳定共同基础阻塞。
- corrected stability继续通过：reserve aggregate median=`0.19385/0.2334/0.218ms`；没有追加批次、修改阈值或删除慢样本。GC和运行顺序metadata完整保留。
- 前置门禁现支持另行授权产品0005，但本轮未实施migration/repository、未运行产品迁移回归或核心矩阵，性能P1/Step5/R-10仍开放，Step6未进入。

## F-009 Step 5 V14 synthetic 稳定性验证 v2（2026-08-30，历史状态）

状态：`step_5_v14_validation_corrected_absolute_performance_gate_failed / awaiting_next_performance_authorization`。

- 失败优先：新增等工作量/all-five-runs契约后先因缺少新符号产生collection error；实现后首次绿测因测试根少一层`validation-01`出现12 failed/2 passed，修正根后两次均`14 passed`，最终复核`14 passed in 1.00s`。
- corrected runner 首次直接执行缺少`backend/src`模块路径，导入阶段停止且未创建数据库；设置进程级PYTHONPATH后执行唯一 validation 批次，没有追加重跑。
- 稳定性比较改为 `101—200 / 501—600 / 901—1000`，五轮聚合 reserve median=`0.1988/0.20785/0.2244ms`，`stable_tail=true`；旧 D1 的37.8%增长是工作量不等价导致，不能继续作为架构失败证据。
- 语义、空间、吞吐通过；recorded p95/p99=`2.3483/3.7771ms`，p95超2ms；baseline p95=`2.1569ms`，ratio=`0.978712`。最终仍为`performance_gate_failed`，但失败原因已从稳定性改为绝对p95。
- 两个 synthetic 文件 ruff、format、mypy通过，最终候选测试14项通过；旧10份migration哈希不变，未创建产品0005、产品根或正式数据库。

## F-009 Step 5 V14 rolling projection D1（2026-08-30，历史状态，已由 v2 修正）

状态：`step_5_v14_projection_preflight_failed / awaiting_revised_projection_design_authorization`。

- 红测：候选模块缺失按预期 collection error；实现后11项通过；新增 v5 重开测试先红，修正候选 exact-table 校验后最终 `12 passed`。
- runner 两次入口导入失败未创建数据库；批次01的 v5 重开问题与批次02的校验时点问题均已纠正，旧数据库不复用。批次03生成最终 metadata-only 摘要。
- 语义和空间通过；完整 in-memory p95/p99、吞吐通过，但 reserve 首/中/末 median=`0.1731/0.21145/0.23855ms`，稳定性失败。
- 两个 synthetic 文件 ruff、format、mypy、tracked/untracked whitespace 通过；旧10份 migration 哈希不变，正式数据库不存在。
- 产品根、control 0005、产品测试和产品 repository 修改均为0；性能 P1、Step 5、R-10继续开放，Step 6未进入。

## F-009 Step 5 V14 rolling projection 实施启动（2026-08-30，当前权威状态）

状态：`step_5_v14_projection_implementation_authorized / synthetic_preflight_pending`。

- 基线预检通过：正式 main 仅五文档；功能分支 33 tracked modified、50 actual untracked、staged 0；两处 HEAD/本地 origin/main 均为 `1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3`。
- 旧 control/observability/business migration 与登记哈希不变；V13 三摘要 SHA256 一致，旧 SQLite 未连接。
- D1/product 两个测试根已预登记但尚未创建；先执行 synthetic 语义、空间和 in-memory 门禁。
- 性能 P1、Step 5、R-10 仍开放，Step 5 综合验收和 Step 6 未进入。

## F-009 Step 5 V14 架构草案（2026-08-30，当前权威状态）

状态：`step_5_v14_architecture_draft_complete / awaiting_v14_projection_prevalidation_authorization`。

- 完成只读基线、10 migration、V13 摘要指纹及源码边界复核；没有运行测试或创建资源。
- 共同成本已分离：in-memory 为 budget window O(n) ledger 扫描；HTTP 为固定多次 control durable commit/单 writer 序列。SQL 重排与换盘均已被证据否定为完整修复。
- 推荐候选 A：ledger 权威、watermark 增量到期、四 scope materialized totals、启动/recovery 原子重建；仅计划 append-only 0005，不改外部契约。
- 候选 B：跨 execution group commit，因 V12 风险和缺少修复后证据不推荐直接实施。
- 下一步仅 D1 synthetic 预验证；性能 P1、Step 5、R-10 继续开放，Step 5 综合验收和 Step 6 未进入。

## F-009 Step 5 V13 存储介质 A/B（2026-08-30，历史状态）

状态：`step_5_v13_storage_ab_complete / awaiting_next_performance_plan_authorization`。离线 Python 3.12 tooling、runner 失败优先契约、E/C 同工作量 A/B 和最终静态/资源审计完成；没有产品、SQL、migration、Dialogue v1、Godot、业务/持久化语义或阈值变更。

- tooling：43 locked packages offline/frozen；网络访问 0，worktree `.venv` 0。
- 契约：V13 新增 8 项，完整文件 23 passed；ruff、format、mypy、diff/whitespace 通过。
- 工作量：E/C 各 1 warm-up + 5 runs，control-off/on 各 100 execution/轮，共 2,400 HTTP；配置 digest 完全一致。
- E: off `p95 67.8951 / p99 103.3343ms / 31.913079/s`；on `p95 166.0149 / p99 348.0677ms / 13.632462/s / 233472 bytes`。相对 p95 增量 `98.1198ms > 30ms`。
- C: off `p95 53.8412 / p99 73.0652ms / 40.498043/s`；on `p95 140.1714 / p99 155.9144ms / 16.158938/s / 237568 bytes`。相对 p95 增量 `86.3302ms > 30ms`。
- 两盘 audit failure=0、integrity/FK 通过、dispatch 归属正确、cost=0、observability SQLite=0；结论 `storage_media_route_not_supported`。
- 性能 P1、Step 5、R-10 未完成；in-memory 与 control-on 相对增量继续开放，未运行 Step 5 综合验收或 Step 6。

## F-009 Step 5 V13 预检（2026-08-30，历史状态）

状态：`step_5_v13_preflight_blocked / awaiting_offline_tooling_resolution_authorization`。

- Git、迁移、V12 证据、目标路径和介质预检通过；C=NVMe SSD、E=USB SSD，可用于真实介质 A/B 命名。
- 没有可直接使用的完整 Python 3.12 测试环境；uv offline dry-run 显示仍需下载 1 包，按授权立即停止。
- 新增资源 0，功能代码/runner/测试修改 0；A/B 红测和 2,400 次 HTTP 对照均未执行。
- 未读取 `.env`、未访问网络/模型/F-005，未删除资源、提交、推送、PR、部署或进入 Step 6。

## F-009 Step 5 V12 回退（2026-08-29，当前权威状态）

状态：`step_5_v12_candidate_reverted / awaiting_storage_ab_validation_authorization`。性能 P1、Step 5 与 R-10 未完成。

- 已撤销四个既有文件中的 V12 transaction batching，并仅移除 `backend/tests/test_control_transaction_batch_step5.py`；此前 F-009 实现和全部临时证据保留。
- Git 文件集合由 `33 tracked + 51 untracked` 恢复为 `33 tracked + 50 untracked`，暂存 0；两处基线未改变。
- ruff/format 4 文件通过，Python 3.12 AST/结构契约通过，V12 标识命中 0，`git diff --check` 通过。由于没有获准创建 Python 3.12 测试环境或新测试根，本轮没有运行 pytest 全回归。
- 已完成 E USB / C NVMe 同工作量 synthetic A/B 方案起草；未创建 C/E 新根、未运行测试、未修改产品、未进入 Step 6。

## F-009 Step 5 V12 性能续修（2026-08-29，当前权威状态）

状态：`step_5_v12_core_gate_failed / awaiting_next_architecture_authorization`。原子批次候选未解决两个授权目标，Step 5/R-10 未完成。

- 红测 3 failed；专项实现 3 passed。取消回归发现 reservation 泄漏后收窄批次，取消+批次 4 passed。控制/预算/契约主集 142 passed、1 failed、53 deselected；唯一失败的内存覆写误落盘修正后聚焦组 5 passed，未在性能失败后重跑整组。
- 五轮：in-memory `4.3027/5.0302ms`；HTTP off p95 `111.2434ms`；HTTP on `293.2776/374.8972ms`，相对增量 `182.0342ms > 30ms`。4 个阻塞失败码，候选失败。
- SQLite observability `104.3828/126.7426ms`、`11.598357/s`、增长 `307200 bytes`，拒绝/吞吐/空间/dispatch/成本继续通过；三 SQLite 仅诊断。
- 未运行 Step 5 综合验收、Step 6、提交、推送或 PR。正式数据库不存在，8000 端口释放。


## Step 0 门禁边界修订与完整矩阵收口（2026-08-29，当前权威状态）

状态：`step_5_gate_boundary_matrix_failed / awaiting_next_performance_authorization`。门禁契约已修订，核心矩阵已完整执行，但性能 P1、Step 5、R-10 仍开放。

- 红测阶段新增边界测试 3 failed；实现后定向契约 14 passed。首次矩阵启动被 TEMP manifest 前置校验拦截，未创建新根；最小修正后相关 2 passed，再执行正式矩阵。
- 五轮阻塞结果：默认 control-on HTTP p95/p99 225.9758/324.5334ms、13.805406/s、增长 237,568 bytes，绝对门禁通过；in-memory p95/p99 3.8461/5.0193ms 失败；control-on 相对 p95 增量 156.7882ms 对允许 30ms 失败。
- 独立 SQLite observability p95/p99 94.7476/118.9803ms、12.456485/s、增长 311,296 bytes，全部通过。三 SQLite诊断 p95/p99 585.708/722.9288ms，两个诊断码不阻塞。
- 完整摘要阻塞失败码仅 3 个：`control_on_relative_p95_regression`、`in_memory_p95_exceeded`、`in_memory_p99_exceeded`。未运行 Step 5 综合验收、Step 6、提交、推送或 PR。
- 66 个新 SQLite integrity/FK 通过；stage、dispatch、成本与资源边界无异常。正式三数据库不存在，8000 未监听。

新矩阵子根为 199 文件、43 目录、19,921,640 bytes；reparse、`.env`、嵌套 Git 均 0。证据保留至 Step 5 收口，Codex 不删除，当前建议继续保留而非手动清理。

## V11 正式运行模式边界验证收口（2026-08-29，当前权威状态）

状态：`runtime_default_slo_passed / awaiting_step0_gate_boundary_decision`。性能 P1、Step 5、R-10 继续开放；没有执行最终矩阵、综合验收、Step 6、提交、推送、PR 或部署。

- 先证伪契约：3 个 V11 测试红测 3 failed；实现仅限 untracked 诊断 benchmark/契约测试后 3 passed，完整契约文件 11 passed。最终定向 3 passed；ruff、format、mypy、两 untracked no-index whitespace 通过。
- 唯一正式默认场景：100 execution、两请求并发、真实 TCP、独立客户端、control-on、business/control SQLite WAL/FULL、NoOp observability。p50/p95/p99 120.4389/157.1363/173.28ms，吞吐 14.865986/s，冻结 250/400ms 与 4/s 门禁通过，稳定失败码 0。
- 归属：100 provider dispatch；control 的 admission/permit/owner/reservation/settlement/join 均 100，completed permit 与 settled reservation 均 100，实际成本 0。business 的 relationship state/event/distinct request 均 100；本普通对话固定集 long-term memory/operation 为 0，不冒充长期写入性能覆盖。
- 两库 integrity ok、FK 错误 0、journal_mode=wal、synchronous=2；observability mode=noop，未创建 observability SQLite。正式/功能六条运行时数据库不存在，8000 与临时端口释放。
- 根因结论：原 `full_loopback_control_on` 将 opt-in SQLite observability 强制叠入正式默认 HTTP。V11 证明默认路径达标；三 SQLite 慢尾仍保留为诊断事实。下一步须由用户确认 Step 0 门禁分层，不继续微优化或擅自修改阈值。
- Git：正式 main 仍仅五份文档；功能分支仍 33 tracked modified + 50 actual untracked、暂存 0；两处 HEAD/origin/main 均 `1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3`。除本轮两个诊断文件外其余 81 文件及 10 migration 指纹不变。

资源：`E:\Agent\cyber-town-f009-step5-tests\performance-v11-runtime-boundary` 当前 7 文件、5 个子目录、576,306 bytes；2 SQLite、4 WAL/SHM 和 1 metadata-only JSON，reparse/.env/嵌套 Git/hidden/read-only 均 0。既有 `tmp_path` 契约测试在授权根的 `tmp` 下自动创建 3 层空 pytest 目录；内容边界未越界，但没有逐层预登记，已作为流程偏差披露且不删除。资源保留至 Step 5 收口，之后由用户手动删除精确根；非 Git 性能证据删除后不可恢复。
...
## 2026-08-27：V7真实磁盘发现response_mapping取消P1，已按约定停止

当前：`step_5_v7_semantic_gate_failed / awaiting_response_mapping_cancel_fix_authorization`。未修复产品、未关闭性能P1/Step5/R10、未进入Step6。

- 预检main五文档/feature33+48/两处1a4fc2c/10migration/V5/V6哈希通过。
- 仅新增f009_step5_attribution.py与test_attribution_step5.py。工具红17+红8后25 passed；磁盘8 passed/1 failed，首次产品失败停止。
- V6 persona修复通过known/missing×queued/running×一次/重复取消8项三库验证：14阶段、complete/cancelled、finish1、dispatch/成本0，重建及重复recovery不变。
- 新P1 V7-CANCEL-RESPONSE-MAPPING：STATE_COMMIT后将response_mapping排队并取消，关闭后仍open/terminal null/12阶段/finish0；provider1、关系event1、预算预留和结算各1、execution_links0。根因是该await在取消try之外。不是重复写入已被证实；其他scope和owner/replay尚缺验证。
- 长期表0，不冒充长期记忆写入覆盖。剩余磁盘场景、profile/plain、最终矩阵、evaluator/digest/quality未运行。
- 2文件ruff/format/mypy/diff/sensitive、ignore通过；81旧文件与10migration指纹不变；功能33tracked+50untracked、正式仍main五文档；正式三类DB均不存在、8000无监听。
- 工具启动两次误拦均已保留证据并纠正：Windows本机socketpair、SQLite精确只读URI；属于harness问题，不能当产品失败。补纯函数URI边界后正式磁盘进程越界尝试0。
- 新V7根30files/13dirs/4,546,560bytes；Step5父根3028/2251/432,934,386，旧V6与四缓存盘点不变。无新venv/授权外缓存，无删除。建议保留失败库和旧证据，待Step5收口后用户手动回收。
- 下一步需要单独授权application/dialogue.py最小取消修复及专项；候选performance-v7-cancel-fix根未创建。完整Prompt和停止条件见[current-task](current-task.md)，精确资源/复现指纹见[evidence](evidence.md)。

以下此前调查和取消修复条目为历史，不覆盖本段。


## 2026-08-27：V7剩余性能只读分析完成，等待补证授权

当前：`step_5_performance_gate_failed / awaiting_v7_attribution_authorization`。性能P1/Step5/R10未完成，Step6未进入。

本轮确认：

- 预检通过：正式main仅五文档；功能指定分支33tracked modified+48untracked；两处HEAD/本地origin/main=1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3；81功能文件和10migration指纹不变。
- V6-CANCEL-01保持fixed/in_memory_regression_verified，113 passed是历史结果；未重跑，不冒充磁盘验收。
- HTTP off p95/p99=435.4531/591.6488ms，on=696.84/872.0176ms；增量261.3869ms>87.09062ms。memory p95=2.7913ms>2，配对no-recorder控制本体也为2.5431ms；吞吐98.37%通过。
- V5预算VM工作量4030→75460，支持扫描增长热点；但其同步profile不能归因V6 worker链路。V6缺少逐operation queue/worker/锁/commit时间线。
- 源码response_mapping await在取消捕获之外，仅登记STATIC-CANCEL-RESPONSE-MAPPING待复现，未认定新P1或实施修复。

推荐：只新增一个V7归因脚本和专项测试，先验证诊断工具、再磁盘取消/恢复/长期写入语义，之后profile+plain补证。现无足够收益证据选择产品修复；SQL、业务连接复用或移锁都需待测量后单独授权。具体范围及Prompt见[任务卡](current-task.md)。

资源：本轮创建0/删除0。Step5根2998files、2237dirs、428387826bytes，V6子集1660files、1323dirs、220710337bytes；四缓存盘点与上轮相同。全部继续保留，用户手动回收政策不变。候选V7根不存在，尚未创建。

本轮没有运行测试、profiling、完整质量入口或CI，没有打开旧SQLite，没有提交/推送/PR/部署。只做Git/哈希/源码/metadata JSON/文档检查。下文历史结果不替代本段。
...
- 生命周期：`F-009_safety_cost_performance / step_5_diagnostic_complete / awaiting_remaining_performance_plan`；P1/Step5仍未完成。
- 最新诊断与有界修复：稳态分库profiler、独立客户端HTTP对照、预算事务内12→4聚合；25新增专项、132相关回归及四文件ruff/format/mypy通过。8组各100HTTP执行均1400stage/100dispatch，完整性/FK通过，零成本；18metadata surface/8sentinel命中0。诊断复测内存p95=3.1644ms、独立HTTP p95/p99=627.1403/724.9823ms，未达到冻结阈值，停止扩大修复。未执行最终五轮完整矩阵/evaluator/digest/全仓质量或Step6。
- 本轮资源：`E:\Agent\cyber-town-f009-step5-tests\performance-v3-diagnostic`，178files/115dirs/22407848bytes；保留诊断证据至Step5收口，由用户手动回收。旧资源未删除/复用；活动worktree保留。正式五文档、功能32tracked+42untracked，HEAD/origin/main仍1a4fc2c；10份migration不变、正式DB不存在、8000及诊断端口释放。下一步等待剩余路径校验/SQL方案确认，不允许直接转入异步架构改造。
...
- F-009 Step 0 已完成：只读确认正式目录 `main == origin/main == 1a4fc2c`，唯一既有差异为候选任务卡，F-008 worktree/分支/缓存保持存在。锁定 8 KiB/四层 JSON 入口、严格 ASCII identity/lowercase UUID、固定注入与 provider 防火墙、五级 token bucket、execution/attempt 预算归属、独立 control SQLite、两次/12 秒 retry、provider+model breaker、控制组件 fail-closed、追加式 observability `0002`、限制性 Dialogue v1 错误扩展和 fake-only 性能门禁。未实现、测试或创建任何临时资源。
- F-009 Step 1 已完成：从 `1a4fc2c` 创建隔离功能 worktree/分支，以模块缺失收集错误建立失败优先基线；完成 8,192-byte/四层/16-item 严格 JSON、固定 ASCII identity/lowercase UUID/message 控制边界、`f-009-input-policy-v1`、固定安全/未来控制 enum 和 DeepSeek 唯一 envelope/usage/model firewall。非法输入均在 provider/状态写入前拒绝，非法 provider response 为不可重试 `provider_invalid_response`。最终相关矩阵 `876 passed`，ruff、86 文件 mypy、schema、17 个变更 Python format、Godot import/unit 及 tracked/untracked diff 全绿；禁止原文命中、未授权 dispatch 和拒绝路径状态写入均为 0。未实施限流、预算、control SQLite/迁移、observability `0002`、retry 或 breaker，未提交/推送/PR。
- F-009 Step 1 资源：删除前测试根 `E:\Agent\cyber-town-f009-step1-tests` 为 696 files/1,078 dirs/60,767,790 bytes，含 676 个隔离业务 SQLite、20 个 synthetic JSON，`.env*`/reparse/nested Git 均为 0；已由用户手动回收，Step 2 预检及当前复核均确认不存在。worktree 内 Godot cache 为 6 files/3,079 bytes；早期门禁在预报告前误建 `.ruff_cache` 5 files/5,674 bytes 与 `.mypy_cache` 4 files/37,548,322 bytes，均保留至 Git 交付后手动处理。正式三类 SQLite 均不存在。
- F-009 Step 2 实现与门禁已通过：连续 refill 五级 token bucket、原子多 scope admission、HMAC peer/scope tag、global/player/player+NPC/conversation provider permit 与独立 STRICT control SQLite `0001` 已接入。最终专项 38 passed、相关矩阵 329 passed、安全范围后端大回归 1082 passed；ruff、32 文件 format、92 source mypy、schema、Godot import/unit、diff、ignore/sensitive 与 migration hash 均通过。rate/control 拒绝零额外 dispatch/三类状态写入，control 原始 scope/peer/payload sentinel 命中 0。
- F-009 Step 2 资源已收口：删除前到期根 `E:\Agent\cyber-town-f009-step2-tests` 为 1,151 files/1,523 dirs/127,051,909 bytes、0 reparse/`.env*`/nested Git，其中误含 80 个 synthetic observability SQLite/WAL/SHM surface。Codex 未删除；用户手动删除精确根后，只读复核确认路径不存在、父目录和 Git/worktree 边界未受影响，且未重建测试根或重跑测试。
- F-009 Step 3 已获授权：固定测试根 `E:\Agent\cyber-town-f009-step3-tests` 于 `2026-08-26T22:13:13.4358635+08:00` 创建，canonical parent 为 `E:\Agent`、0 reparse；仅允许 synthetic pricing/budget、pytest/TEMP、隔离业务/control SQLite 与 append-only observability `0002` 所需 metadata-only SQLite，完成后由用户手动回收。
- F-009 Step 3 的未授权 `.venv` 阻断已关闭：用户手动删除 `E:\Agent\comprehensive-cases\15-cyber-town-f009\.venv` 后，Codex 只读复核确认不存在，后续只复用正式目录既有 Python 3.12 环境。防复发规则保持为：无既有 `.venv` 时禁止用 `uv run` 探测解释器。
- F-009 Step 3 已完成：新增 `f-009-budget-policy-v1`、固定 attempt/cost windows、micro-USD 向上取整、原子 reservation/dispatch/settlement/release、可信 usage 差额释放、异常/取消/restart 保守结算，以及 append-only control/observability `0002`。真实 provider 无显式批准价格表时 composition 在创建 provider/数据库前 fail-closed；budget hard reject 为 429/nonretryable/no Retry-After；recorder 故障 fail-open，结算故障阻断业务写入。
- F-009 Step 3 门禁：失败优先为 1 个缺模块收集错误；专项 `26 passed`；最终安全范围后端 `1123 passed`，配置 `353 passed / 1 deselected`（只排除实际创建 `.env` 的用例）。offline lock、schema、ruff、94 source mypy、36 Python format、Godot import/unit、diff、ignore/sensitive、migration hash、正式数据库和端口门禁全绿；重复计费、重复结算、额外 dispatch、结算失败业务写入、scope 泄漏与禁止原文命中均为 0。
- F-009 Step 3 资源已到期并保留：`E:\Agent\cyber-town-f009-step3-tests` 最终为 1,673 files/2,211 dirs/161,147,765 bytes，含 1,556 SQLite、45 WAL、45 SHM、27 JSON，0 `.env*`/reparse/nested Git/hidden/read-only；Codex 未删除，建议用户仅手动删除该精确目录并由 Codex 只读复核。worktree 的 ruff/mypy/Godot 缓存继续保留至 Git 交付。
- F-009 Step 4 已完成：实现固定 2-attempt/5 秒单次/12 秒总 deadline、200ms+0..200ms jitter 的 timeout/unavailable retry，取消/迟到保护，以及 provider kind+model scope 的持久化 closed/open/half-open breaker。control/observability 均只追加 STRICT `0003`；503 `circuit_open` 使用受限 Retry-After，Dialogue v1 成功字段与 Godot UI 不变。
- F-009 Step 4 门禁：失败优先为 1 个缺符号收集错误，最终专项 `18 passed`、相关回归 `123 passed`、安全范围后端 `1141 passed`、配置 `353 passed / 1 deselected`。ruff、38 Python format、96 source mypy、schema、Godot import/unit、tracked/18 untracked diff、ignore/sensitive、migration、正式数据库和端口门禁全绿；scope 泄漏、禁止原文、重复计费/写入、超 2 次 retry amplification 与未授权 dispatch 均为 0。
- F-009 Step 4 资源已到期并保留：`E:\Agent\cyber-town-f009-step4-tests` 为 1,269 files/1,738 dirs/145,773,129 bytes，含 1,203 SQLite、24 WAL、24 SHM、18 JSON，0 `.env*`/reparse/nested Git/hidden/read-only；Codex 未删除，建议用户仅手动删除精确根。worktree 的 ruff/mypy/Godot 缓存继续保留。
- F-009 Step 5 已获授权：Step 4 根当前不存在且未推断删除主体；固定测试根 `E:\Agent\cyber-town-f009-step5-tests` 于 `2026-08-27T00:32:05.5555323+08:00` 创建，canonical parent 为 `E:\Agent`、0 reparse、创建后为空，仅允许 synthetic evaluator/performance/三进程 metadata 与隔离 SQLite。
- F-009 Step 5 性能门禁失败并停止：综合 evaluator/性能契约失败优先后为 `23 passed`；实际 warm-up + 5-run 基线确认 fresh observability 每 100 trace 增长 `671,744 bytes`（阈值 512 KiB），完整三 SQLite loopback control-off 为 p95/p99 `1257.8538/1483.6411ms`、`1.949754/s`，control-on 为 `1843.02425/2095.8564ms`、`1.063318/s`，均越过冻结门禁；control-on p95 相对增加 `585.17045ms`，超过允许值 `251.57076ms`。按缺陷规则未继续故障组合、三进程输出或完整回归，状态为 `step_5_performance_gate_failed / awaiting_fix_authorization`。
- F-009 Step 5 当前资源：`E:\Agent\cyber-town-f009-step5-tests` 为 49 files/26 dirs/24,557,647 bytes，含 48 个隔离 SQLite 和 1 个 metadata-only JSON，`.env*`、reparse、nested Git 均为 0；Codex 未删除。worktree 缓存为 ruff 5 files/6,682 bytes、mypy 3 files/38,056,165 bytes、Godot 6 files/3,079 bytes。正式三类 SQLite 不存在，8000 无 listener。
- 未覆盖：NPC 自主互动、Qdrant/embedding、正式 UI/素材、真实模型自动红队、生产压测与部署均不在 F-009 范围。
- F-009 Step 5 最小修复调查：4 execution 的 profiling 为 127 connect/close、123 commit；提交/关闭 `0.865/0.670s`，stage progress `1.028s`，确认连接/事务热点。固定 schema 的 6 种页布局最小增长仍 `581,632 bytes > 512 KiB`；control DB 对照差额 `311,296 bytes > 256 KiB`。未找到授权内的诚实最小空间修复，已在修改产品代码前停止，等待是否允许兼容 append-only 索引迁移的单独决定；未放宽阈值。
- F-009 Step 5 修复调查资源：根现为 71 files/29 dirs/32,672,604 bytes，66 SQLite + 3 JSON + 1 WAL + 1 SHM，env-like/reparse/nested Git 均为 0。新增路径 `profile-before`、`profile-before-2`、`storage-probe` 及两份 metadata-only 摘要均在原授权根内；建议暂留作为缺陷证据，不删除。

## D0 历史收口（已被下方 D1 结论取代）

F-009 当前为 `step_5_storage_diagnostic_complete / awaiting_layout_prevalidation_authorization`。用户授权继续后按已说明的 D0 边界完成物理空间诊断，没有实施候选 A/B、产品迁移或连接修复。新的候选主文件增长 264/644/908 KiB、占用增长 280/668/948 KiB 仍超限；旧 280/672/952 KiB 作为独立历史观测保留，不用数页波动宣称改善。

此前索引预验证只新增诊断脚本/纯函数测试，红测 1 个缺模块收集错误、最终 3 passed，ruff/format/2 source mypy/whitespace 通过。两组各 100 completed、100 dispatch、1400 stage、300 cost/200 retry-breaker event，fake 成本和 13 metadata surface 禁止原文 sentinel 命中均为 0；8 类查询结果一致，retention COUNT 改为扫描。这里是历史结果；D0 的新增测试/新实验库结果见下文，旧证据数据库本轮未打开。

本轮新增两个 D0 诊断/测试文件，红测 1 个模块缺失收集错误；12 项 D0 + 3 项原索引纯函数共 15 passed，ruff/format/2 source mypy/diff 通过。两组各 100 completed/dispatch、1400 stage、300 cost/200 retry-breaker，行数/完整性/页字节守恒、fake 成本 0，5 个新 metadata surface 的 110 个 sentinel 命中 0。没有运行完整性能矩阵、evaluator/三进程/故障组合或完整回归。

control 表/约束索引/非唯一索引增长为 180/76/24 KiB，observability 为 360/232/76 KiB。八叶表 PK 索引增长 control 52 KiB、observability 112 KiB；即使暂假设后者全可消除，本轮 156 KiB 缺口仍差 44 KiB，且新的主键表/二级索引成本尚未测量。建议下一次仅批准任务卡 D1：固定八叶表、原十索引、4096 页的 synthetic WITHOUT ROWID 预验证，失败即停，不保证足够；D1 通过也不自动创建产品 0004 或修复连接。

资源：新 `E:\Agent\cyber-town-f009-step5-tests\storage-breakdown-v2` 为 7 files/3 dirs/3,481,274 bytes，6 synthetic SQLite + 245,434-byte metadata JSON；无 WAL/SHM，7 文件独占打开复核无占用。整个 Step 5 根为 93 files/36 dirs/39,557,163 bytes；旧 index-v4-preflight 盘点未变。ruff/mypy/Godot cache 为 5/3/6 files、6,682/38,158,565/3,079 bytes，mypy 的 16-byte missing_stubs 标记经工具正常维护消失，未执行清理命令。建议暂留诊断证据；若放弃 D0，可由用户手动删除精确新子根，命令与不可恢复性见任务卡，不删除父根/旧证据/缓存。

正式仍五份文档，功能 31 tracked + 30 untracked；原 59 文件指纹一致，产品 0004 不存在，八份旧 migration 与正式三类数据库不存在的边界保持。无新 venv/pytest cache/外部服务/真实模型/F-005 访问。P1/Step 5 仍阻塞，不进入 Step 6、提交、推送、创建 PR、部署或删除资源。

## 当前 D1 结论与下一批准动作

**候选 B 空间预验证失败，已停止。** 4096 页、固定八叶表 WITHOUT ROWID + 原十个非唯一索引，未扩大清单。两组三库固定输入/ID/时钟/顺序的 100 unique execution 对照：候选主文件增长 control/observability/combined 为 204/588/792 KiB；排除 freelist 后占用为 **232/632/864 KiB**，后两项分别超出冻结阈值 120/96 KiB。业务各增长 60 KiB，不计 combined。基线增长 304/944/1248 KiB；不能以表面节省取代绝对门禁。

初始红测缺模块；最终 D1 33 + D0 12 + index 3 = **48 passed**，ruff/format/2 source mypy/diff 通过。布局及所有逻辑约束保持、逐列变异/重复/NULL 拒绝、原子回滚验证通过。各组 100 completed/dispatch、1400 stage、300 cost/200 retry-breaker、100 durable-open 和 1300 progress 独立连接读回；三库逻辑行 digest 配对一致，仅排除迁移台账建库时间。restart 验证 9 个增量阶段后只收口 1 abandoned，完整 trace 不变，重复 recovery=0。6 metadata surfaces/110 sentinel 命中 0，FK/stage 异常和 fake 成本 0。

首轮诊断漏排除业务迁移 `applied_at`，通过 1 failed/2 passed 红测修正诊断器后使用新 `paired-fixed` 子目录重跑，未复用/覆盖初轮库。初轮中止产物不作验收。未关闭 P1/Step 5，不进入 D2/Step 6，不实施产品 0004、连接修复、完整矩阵或后续 evaluator/三进程/故障组合。下一步等待用户重新确认存储方案边界，不默认放宽阈值或扩大八表/十索引，也不承诺现边界下必然可行。

资源：`E:\Agent\cyber-town-f009-step5-tests\layout-preflight-v2` 为 17 files/8 dirs/6526123 bytes；有效 `paired-fixed` 为 8/3/3572875，首轮三个目录合计 2953248 bytes。总根含 14 SQLite、234635-byte summary、初轮遗留 WAL 32/SHM 32768 bytes；17 文件无占用，0 reparse/env-like/nested Git/hidden/read-only。Step 5 父根 110/45/46083286；旧 D0/index 证据盘点未变。缓存 ruff/mypy/Godot 为 5/3/6 files、6682/38199525/3079 bytes，继续保留。建议按期限保留，用户可决定手动丢弃首轮非验收子目录；完整命令与不可恢复性见任务卡末节，Codex 不代删。

正式 main 仅五文档；功能原 61 文件指纹不变、当前 31 tracked + 32 untracked，仅新增授权两个 D1 文件。两处 HEAD/origin/main 仍 `1a4fc2c`、八旧迁移 hash 不变、产品 0004 不存在；正式/功能三类正式数据库不存在，8000 无 listener。有效摘要 SHA-256 `8e9feef5a6d18b38983027fb275020afc04b59d4d7756a1e80d93babdbdf72e0`。未访问 .env、外部服务/F-005，未提交/推送/PR/部署/删除。

## P1 续修授权后的调查进展

再次核对当前代码、D1 summary及profile-before-2 metadata证据，基线/31+32/八旧迁移仍一致。空间热点是重复的阶段/结果/policy文字及HMAC/UUID表示，延迟存在每操作connect/close热点。提出任务卡末节候选C：在原四observability叶表做可逆紧凑编码，control沿用B；不扩大八表/十索引，不改变14-stage/业务/API/CLI或阈值。收益仍需新synthetic预验证，不能先宣称可达标。

C会改变指定SQLite列类型和CHECK物理表达，触及用户此前明确禁止的字段编码边界，尚需明确确认；本轮只更新五文档，没有改产品、测试、新资源或SQL连接。下一步应确认C的精确例外及预验证→产品迁移→连接修复→完整门禁顺序，不是再次无界试页大小，也不是直接进入Step6。完整目标保持未完成，现有测试资源和缓存继续保留，Codex不删除。

## 候选 C 获批后的预验证（执行中）

用户已明确批准物理编码例外，当前 `step_5_compact_prevalidation_in_progress / candidate_c_authorized`。预检基线、原63文件集合、八旧迁移一致。codec 红测为缺模块；实现和约束/adapter 测试57项通过，加既有诊断48项共105 passed，ruff/format/mypy通过。waiter 测试 fixture 初次缺 inflight_shared，既有 DTO 拒绝后只纠正 fixture，未改业务不变量。

新根 `E:\Agent\cyber-town-f009-step5-tests\compact-preflight-v1` 于13:29:36创建，tmp及paired-fixed子路径已登记。仅 synthetic/FakeProvider/metadata；当前固定100次执行三库对照进行中，尚无空间通过结论。不注册产品0004、不改既有仓储或事务、不删除旧证据，继续保留至Step5收口由用户手动回收。
## C续修：空间问题已改善，性能P1仍阻塞（最新）

用户明确批准C后，synthetic空间/语义预验证232/420/652KiB通过；两份append-only0004与有界连接复用已实施。旧八迁移哈希、业务/API/Godot与14-stage持久化/同步强度不变。定向105项空间/语义、132项迁移/连接/控制及8项测量测试通过，非最终全量质量。

核心warm-up+5-run失败：HTTP control-on p95/p99=787.5748/853.159ms，超过250/400；相对增量260.904ms超过105.33416；in-memory+control5.0921/7.1039ms超过2/5。SQLite observability133.9627/151.7158ms、9.027181/s通过；同工作量memory吞吐约90.27%通过。五项失败码已留metadata摘要，未改阈值继续试跑。其余矩阵、evaluator、三进程digest、故障组合和最终回归未执行，不进入Step6。

正式main五文档，功能32tracked+40untracked，HEAD/本地origin均1a4fc2c；未Git交付。新性能metadata152surface/11sentinel命中0，正式三类SQLite不存在、8000空闲。Step5根692files/449dirs/124839407bytes，新增C/迁移/性能根共78756121bytes，旧证据与缓存保留；补列旧backend/.mypy_cache，不是本轮创建。具体数量、Git/敏感类别、生命周期和用户手动命令见任务卡，Codex未删除。P1/Step5/R10均未完成，等待修订性能方案。
# 2026-08-30 — F-009 Step 5 V16 rolling projection 统一协议重验启动

- 已只读确认正式 main 仅五份项目文档修改；功能 worktree 位于 `feat/f-009-safety-cost-performance`，HEAD/local origin/main=`1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3`，33 tracked modified、52 actual untracked、staged 0，产品 `0005` 不存在。
- 已预登记 `E:\Agent\cyber-town-f009-step5-tests\performance-v16-budget-projection-unified` 及精确 surface；尚未创建。父目录存在且非 reparse point，既有 V13 Python 3.12 工具环境可用。
- 下一步仅做失败优先测试、synthetic 语义/空间/统一性能重验；不实施产品或后续阶段。
- 失败优先：缺少V16契约时1个collection error；实现后夹具前置条件两次暴露并修正，最终`18 passed`。ruff、format、mypy（显式MYPYPATH）与diff通过。
- 唯一V16批次：语义16/16、integrity/FK通过；control增长196608/262144 bytes；reserve稳定性通过；no-recorder p95/p99=2.7582/3.6022ms，recorded=3.2393/4.215ms，吞吐比0.805846。最终`performance_gate_failed`，未运行额外性能批次或产品实现。
- 当前状态：`step_5_v16_projection_unified_performance_gate_failed / awaiting_next_performance_authorization`。
# 2026-08-30 — F-009 Step 5 V17 rolling projection 阶段归因启动

- 已只读确认正式main仅五份项目文档修改；功能worktree为`feat/f-009-safety-cost-performance`，HEAD/local origin/main=`1a4fc2cdf142b00f823bebf9b2abe6e74ba23ab3`，33 tracked modified、52 actual untracked、staged 0。
- 产品`0005`不存在；10份migration未改动；V16摘要SHA256=`CCF52CEDB26FBB7A49E59EDAB497D182699C2D5172722AB546361595AEB7BF1D`。
- V17精确surface已展开并预登记，创建前根不存在、父目录非reparse。下一步只做失败优先stage attribution测试和唯一12000-execution companion。
- 失败优先与工程门禁：缺少V17入口时1个collection error；实现后最终`21 passed`。ruff、format、2-source mypy、tracked/untracked whitespace通过。
- 唯一V17 companion完成：五轮overall p95 baseline=`2.062/2.2362/2.9959/3.1809/3.4638ms`，recorded=`2.1328/2.9451/3.6307/3.007/3.0111ms`；aggregate=`2.9959/3.007ms`。归属100%、线程1→1、provider dispatch每run=1000。
- 阶段结论：budget reservation是最大单项热点，recorded p95=`0.9746ms`；recorder直接stage/terminal p95增量仅`0.023/0.0004ms`；未归属中位约`0.05ms/execution`。推荐下一步仅做set-based projection delta synthetic候选A。
- 当前状态：`step_5_v17_projection_stage_attribution_complete / awaiting_v18_candidate_authorization`；产品`0005`、Step5综合验收和Step6均未进入。
